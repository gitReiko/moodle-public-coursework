...
class SectionEdit extends SectionAction 
{
    private $section;
    ...
    {
        $inputs = '<input type="hidden" name="'.ConfigurationManager::DATABASE_EVENT.'" value="'.TasksManagement::EDIT_SECTION.'"/>';
        $inputs.= '<input type="hidden" name="'.SECTION.ID.'" value="'.$this->section->id.'">';
        return $inputs;
    }

    private function get_section() : stdClass
    {
        $sectionId = optional_param(SECTION.ID, null, PARAM_INT);
