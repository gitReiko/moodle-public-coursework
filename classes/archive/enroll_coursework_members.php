...
// Enrollment of members in the module
class EnrollCourseworkMembers
{
    ...
    // Themes management gui
    private function gui_themes_management() : string
    {
        $str = '<h3>'.get_string('coursework_themes_management', 'coursework').'</h3>';
        $courses = array_unique($this->courses);

        $str.= '<ol class="theme_creation">';
        foreach($courses as $course)
        {
            $str.= '<li><h4>'.$this->get_course_name($course).'</h4>';
            $str.= $this->add_new_theme_gui().'</li>';
        }
        $str.= '</ol>';
    ...
    private function get_course_name(int $course) : string
    {
        global $DB;
        $course = $DB->get_record('course', array('id'=>$course), 'fullname');

        if(isset($course->fullname)) return $course->fullname;
        else return 'Course doesnt have fullname';
    }

    private function add_new_theme_gui() : string
    {
        $str = '<ol><li>';
        $str.= '<form>';
